...
# Delete all existing files in the Space first
try:
    ...
    if files:
        ...
        if operations:
            api.create_commit(
                ...
                commit_message="Clear Space before sync",
            )
            print(f"Deleted {len(operations)} existing files")
except Exception as e:
    print(f"Warning: could not clear existing files: {e}")
